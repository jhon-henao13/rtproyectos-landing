        ...
        <div className="hidden md:flex items-center space-x-10">
          {navLinks.map((link) => (
            <a 
              key={link.name} 
              href={link.href} 
              className="text-slate-800 hover:text-brand-primary text-base font-medium transition-colors duration-300 relative after:absolute after:bottom-[-6px] after:left-0 after:w-0 after:h-[2px] after:bg-brand-primary hover:after:w-full after:transition-all after:duration-300"
            >
              {link.name}
            </a>
          ))}
        ...
        <div className="md:hidden absolute top-24 left-0 w-full bg-white border-b border-slate-100 px-6 py-6 space-y-4 shadow-xl animate-fadeIn">
          {navLinks.map((link) => (
            <a 
              ...
              className="block text-slate-800 hover:text-brand-primary font-medium text-base py-3 border-b border-slate-50 last:border-0"
            >
              {link.name}
            </a>
          ))}
